  ...
  <div class="p-6 bg-white h-full flex flex-col">
    <div class="flex justify-between items-center mb-5 pb-2.5 border-b border-gray-200">
      ...
      <button @click="clearChat"
        class="px-4 py-2 bg-rose-100 hover:bg-rose-200 border border-rose-400 rounded text-rose-700 transition-colors">
        Clear Chat
      </button>
    </div>

    <div class="flex-1 overflow-hidden border border-gray-300 rounded bg-gray-50 flex flex-col">
      <div class="flex-1 overflow-y-auto overflow-x-hidden py-2.5" ref="messagesContainer">
        <div class="container mx-auto w-200">
          ...
          <div v-for="message in chatStore.messages" :key="message.id" :class="[
            ...
            <div>
              <pre v-if="message.role === 'assistant'"
                class="m-0 whitespace-pre-wrap break-words max-w-full font-inherit">{{ message.content }}</pre>
            ...
            <div v-if="message.tools && message.tools.length > 0" class="mt-4 pt-4 border-t border-gray-200">
              ...
              <div v-for="(tool, index) in message.tools" :key="index" class="rounded mb-2.5 font-mono">
                <strong class="block mb-1 text-blue-600">{{ tool.name }}</strong>
                <pre
                  class="my-1 text-xs border border-gray-200 rounded whitespace-pre-wrap break-words max-w-full"><code class="language-json" v-html="highlightCode(JSON.stringify(tool.arguments, null, 2))"></code></pre>
                <div v-if="tool.result" class="mt-2.5 pt-2.5">
                  <strong class="block mb-1 text-blue-600">Result:</strong>
                  <pre
                    class="my-1 text-xs border border-gray-200 rounded whitespace-pre-wrap break-words max-w-full"><code class="language-json" v-html="highlightCode(JSON.stringify(tool.result, null, 2))"></code></pre>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="chatStore.isLoading"
            class="mx-2.5 p-4 rounded-lg max-w-[80%] break-words bg-gray-100 mr-auto ml-2.5">
            <div class="flex justify-between items-center mb-2 text-xs opacity-80">
      ...
      <div class="border-t border-gray-300 bg-white p-4">
        ...
        <div class="flex gap-2.5 items-end mx-auto w-180">
          <textarea v-model="newMessage" @keydown="handleKeyDown"
            ...
            placeholder="Type your message... (Shift+Enter for new line, Enter to send)" rows="3"></textarea>
